...
//
// This page is dev-only: it is the sole place that globs the `-anim-` sources,
// and it is deliberately absent from build.rollupOptions.input, so those
// near-duplicate exports cannot reach a production bundle. Saving POSTs to the
// `editor-save` dev middleware in vite.config.ts.
//
// The preview drives the real PicoCadAnimator over the real loader/material, so
...
import { clipDuration, retro } from './three';
import { createAnimPreview } from './three/animPreview';
...
// The game renders at 0.25; in a 320px panel that is ~80px of picture, which is
// too coarse to judge a motion by. Page-local — the game's scale is untouched.
retro.scale = 0.5;
...
const preview = await createAnimPreview(previewCanvas).catch((error: unknown) => {
    previewEmpty.textContent = `renderer init failed: ${error}`;
    throw error;
});
let previewToken = 0;

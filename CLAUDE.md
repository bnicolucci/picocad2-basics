...
- Entity editor palette: one dropdown under the Solid Colour swatches — "Current Model Palette" then the 22 themes in `assets/palettes/picocad_palettes.ts`. A model texture is palette-INDEXED, so switching rebuilds only the 16x3 colour map and leaves the index pixels alone; the 3D preview, the Solid Colour swatches and the UV picker therefore recolour together, because all three read that one texture. `transparentIndex` stays the MODEL's — it says which texels were authored to vanish, which is a property of the texture, not the colours. Editor-only so far: the choice is not written to the blueprint

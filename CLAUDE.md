...
src/
  ...
  sounds.ts      the game's sounds + song, as numbers; play/playAt/playMusic
...
| `src/sounds.ts` | The game's own sounds: a name → `{note, instrument}` registry (typed, so a stale `play('...')` is a compile error) plus the song. `play` / `playAt(name, x, y, z)` — which pans by the camera's right vector and falls off with distance — and `playMusic`. `preloadSounds()` in `init()` pays the few ms per sound up front instead of dropping a frame mid-game. Instruments are SoundBox (`sb.bitsnbites.eu`) parameter order. |

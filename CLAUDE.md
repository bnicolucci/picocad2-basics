...
**Audio**
- Synthesis is pure: `renderSound`/`renderSong` take numbers and return Float32
  buffers, touching no `AudioContext` — which is what makes them testable `(test)`
- The `AudioContext` is created on the first click/keypress; `play()` before that
  is a silent no-op, so nothing in the app needs a click-to-start screen
- Instrument arrays are SoundBox's 29-parameter order, so its exports paste in
  unchanged. The port is bit-identical to q1k3's synth — verified sample-for-sample
  against `wip/q1k3-master/source/audio.js`, but that folder is gitignored so it
  is not a standing test
- A delay TIME of 0 with a non-zero amount is not "no delay" — the channels
  cross-feed in place. Do not short-circuit `applyDelay` on a zero shift `(test)`
...
- Entity editor: Save keeps you on the entity AND part you were editing. Saving rewrites a file the page imports, so Vite reloads it — the selection is handed across in `sessionStorage`, keyed by entity NAME so it survives reordering. Without this the reload silently drops you on the first entity, which is invisible while there is only one and maddening once there are two
...
- **A part's SCALE does not inherit** `(test)`. A part is a node holding position + rotation, with its mesh *inside* it holding the scale — so squashing a cube into a slab does not squash the cylinder bolted to it, while position and rotation still carry down (which is the point of parenting). This deliberately departs from normal scene-graph semantics; do not "fix" it back. To scale a whole entity, scale the group `instantiateEntity` returns. Consequence: `group.children[i]` is a part NODE, and its mesh (the thing carrying `.model`) is that node's child
...
- Entity palette: `EntityBlueprint.palette` is an id from `assets/palettes/picocad_palettes.ts`, chosen in one dropdown under the Solid Colour swatches ("Current Model Palette" first). A model texture is palette-INDEXED, so `repaletteTexture` rebuilds only the 16x3 colour map and leaves the index pixels alone `(test)`; the 3D preview, the Solid Colour swatches and the UV picker therefore recolour together, because all three read that one texture. `transparentIndex` stays the MODEL's — it says which texels were authored to vanish, a property of the texture, not the colours `(test)`
- Recoloured uploads are cached per (model, palette) `(test)`, so parts sharing a mesh still share ONE upload — otherwise naming a palette would silently cost a draw call per part. A palette id that no longer exists means the models keep their own colours rather than throwing. The generated module imports the palette registry only when some entity uses one
...
src/
  ...
  sounds.ts      the game's sounds, as numbers; play/playAt/playMusic
...
| `src/lib/audio.ts` | Sound and music synthesised from numbers — a TypeScript port of the Sonant-X fork phoboslab wrote for js13k (zlib; notice kept in the file). Oscillators come out of one shared 4096-entry table. `renderSound(note, instrument)` / `renderSong(song)` are pure and return `Float32Array`s; `play(samples, {volume, pan, loop})` is the only part that needs an `AudioContext`, and that is created lazily on the first user gesture. |
| `src/sounds.ts` | The game's own sounds: a name → `{note, instrument}` registry (typed, so a stale `play('...')` is a compile error). `play` / `playAt(name, x, y, z)` — which pans by the camera's right vector and falls off with distance — and `playMusic(song, volume)`. `preloadSounds()` in `init()` pays the few ms per sound up front instead of dropping a frame mid-game. Instruments are SoundBox (`sb.bitsnbites.eu`) parameter order. |
| `src/assets/songs/*.ts` | CONVERTED from SoundBox / Sonant-X JSON exports: the named instrument fields become the 29-number `Instrument` tuple, `p` the bar sequence, `c[].n` the 32-row patterns. `seconds` is the exact bar count, not the export's rounded `songLen`, so the buffer ends where the loop restarts. One module per song, so unused ones tree-shake out. Rendering costs roughly 15ms per track-minute on the calling thread — fine for a short loop, a visible freeze for a long one. |
...
| `src/lib/editorTexture.ts` | Showing a model's texture in an editor. The texture is palette-indexed, so `textureRgba` expands it to RGBA through palette row 0 (the transparent index becomes alpha 0); `tileGrid` / `tileAtPixel` do the 1-based column/row maths `uv.tile` uses. DOM-free (no `ImageData`) so it stays testable — the caller wraps the bytes `(test)`. |

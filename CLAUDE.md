...
src/
  ...
  assets/        models + primitives (.txt), generated *_animations.ts clips,
                 GENERATED entities.ts registry, palettes/
```
...
| `src/lib/renderer.ts` | Minimal WebGL2. `render(viewProj, lightDir, instances)` uploads models on first sight (cached) and sorts by model to minimize texture rebinds; per-instance uv/color-override/per-mesh matrices; palette shading with checker dither; `renderer.retroScale` for the pixelated upscale; `setBackground(hex)`. |
